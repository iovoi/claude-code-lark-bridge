...
- [ ] **T1.1** Create the `bridge/` package skeleton.
  ...
  - Depends on: —
- [ ] **T1.2** Extend `feishu_api.py` with card primitives.
  ...
  - Depends on: T1.1
- [ ] **T1.3** Rewrite `pyproject.toml` for the new package.
...
- [ ] **T2.1** Implement NDJSON line framing + subprocess spawn.
  ...
  - Depends on: T1.1
- [ ] **T2.2** Implement the control protocol: initialize + request/response correlation.
  ...
  - Depends on: T2.1
- [ ] **T2.3** Implement user-turn send, event stream, interrupt, teardown.
...
- [ ] **T3.1** Define `AgentAdapter` + the `AgentEvent` union.
  ...
  - Depends on: T1.1
- [ ] **T3.2** Implement `ClaudeAdapter` event mapping + turn loop.
...
- [ ] **T3.3** Wire approvals: route inbound `can_use_tool` → allowlist → approval callback.
...
## Phase 4 — Lark layer
- [ ] **T4.1** Move + extend inbound ingest (thread_id + card actions).
  ...
  - Depends on: T1.1
- [ ] **T4.2** Implement the Lark wrapper + emoji cycle.
  ...
  - Depends on: T1.2
- [ ] **T4.3** Implement the streaming-card renderer.
...
- [ ] **T5.1** Implement session store.
  ...
  - Depends on: T1.1
- [ ] **T5.2** Implement `ScopeRunner` (single-flight + phase machine).
...
- [ ] **T5.3** Implement approvals end-to-end + stuck watchdog.
...
- [ ] **T5.4** Implement the runtime event loop + CLI.
  ...
  - Depends on: T5.3
- [ ] **T5.5** Implement cross-platform supervisor (up/status/stop).
...
- [ ] **T6.1** Rework installers for the new package.
  ...
  - Depends on: T5.5
- [ ] **T6.2** Update docs + plugin surface.
  ...
  - Depends on: T6.1
- [ ] **T6.3** Remove the old MCP/PTY layer.
...
- [ ] **T7.1** Build the fake-claude stub.
  ...
  - Depends on: T2.3
- [ ] **T7.2** End-to-end adapter test via the stub (acceptance #12).
